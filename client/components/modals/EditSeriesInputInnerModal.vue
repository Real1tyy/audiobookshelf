        ...
        <div class="bg-bg rounded-lg px-2 py-6 sm:p-6 md:p-8" @click.stop>
          <div class="flex">
            <div class="grow p-1 min-w-48 sm:min-w-64 md:min-w-80">
              <ui-input-dropdown ref="newSeriesSelect" v-model="selectedSeries.name" :items="filteredSeriesNames" :disabled="!isNewSeries" :label="$strings.LabelSeriesName" show-all-when-empty @input="seriesNameInputHandler" />
            </div>
...
export default {
  ...
  computed: {
    ...
    filteredSeriesNames() {
      // Show first 5 series names by default, ui-input-dropdown will handle search filtering
      return this.existingSeriesNames.slice(0, 50)
    }
  },
  methods: {
