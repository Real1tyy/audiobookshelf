<template>
  <div ref="wrapper" class="relative" v-click-outside="clickOutside">
    ...
    <div v-show="showMenu" class="absolute z-50 mt-1 w-full bg-bg border border-black-200 shadow-lg rounded-md py-1 ring-1 ring-black/5 overflow-auto focus:outline-hidden text-sm max-h-96">
      <ul class="h-full w-full" role="menu">
        <li v-for="item in selectItems" :key="item.value" class="select-none relative py-2 pr-9 cursor-pointer hover:bg-white/5" :class="item.value === selected ? 'bg-white/5 text-yellow-400' : 'text-gray-200 hover:text-white'" role="menuitem" @click="clickedOption(item)">
  ...
  computed: {
    ...
    selectItems() {
      return [
        ...
        {
          text: this.$strings.LabelFinished,
          value: 'finished'
        },
        {
        ...
        {
          text: this.$strings.LabelNotStarted,
          value: 'not-started'
        },
        {
    ...
    selectedText() {
      if (!this.selected) return this.$strings.LabelAll
      const item = this.selectItems.find((i) => i.value === this.selected)
      return item ? item.text : this.$strings.LabelAll
    }
  },
  methods: {
    clearSelected() {
